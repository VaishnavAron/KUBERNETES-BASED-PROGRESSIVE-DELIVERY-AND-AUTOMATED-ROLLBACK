Write-Host "[1/5] Checking Kubernetes Cluster..." -ForegroundColor Cyan
kubectl cluster-info

Write-Host "`n[2/5] Starting Argo Rollouts Dashboard (Port 3100)..." -ForegroundColor Cyan
Start-Process -NoNewWindow -FilePath "kubectl" -ArgumentList "argo rollouts dashboard"

Write-Host "[3/5] Starting Prometheus Port-Forward (Port 9090)..." -ForegroundColor Cyan
Start-Process -NoNewWindow -FilePath "kubectl" -ArgumentList "port-forward svc/prometheus 9090:9090 -n monitoring"

Write-Host "[4/5] Starting Grafana Port-Forward (Port 3000)..." -ForegroundColor Cyan
Start-Process -NoNewWindow -FilePath "kubectl" -ArgumentList "port-forward svc/grafana 3000:3000 -n monitoring"

Write-Host "[5/5] Starting Rollouts-Demo App Port-Forward (Port 8080)..." -ForegroundColor Cyan
